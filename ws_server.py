...
协议见 docs/PROTOCOL.md（v2.0 流式）：hello 握手 / 心跳 / ask / 流式响应。
- ask（C→B）：{"type":"ask","id":...,"text":"...","lang":"zh-CN"}
  ASR 已移至 C 端（sherpa-onnx 本地识别），C 端识别后直接发 ask 文本帧。
- voice_ask 已弃用（返回 asr_unavailable 错误码）。
- 流式响应（B→C，v2.0）：
  stream_begin → stream_delta × N → stream_end
  - {"type":"stream_begin","id":...,"recognized":null}
...
class FairyWsServer:
    ...
    async def _handle_voice_ask(
        ...
        """voice_ask 已弃用：ASR 已移至 C 端（sherpa-onnx 本地识别），C 端应直接发 ask。"""
        req_id = data.get("id") or str(uuid.uuid4())
        await self._send_error(ws, req_id, "asr_unavailable", "ASR 已移至 C 端，请直接发 ask 帧")
